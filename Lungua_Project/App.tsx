...
const App: React.FC = () => {
  ...
  const LocationSharerBanner = () => {
    ...
    const bannerContent = {
        ...
        countingDown: { bg: 'bg-red-900', border: 'border-red-500', text: 'text-red-200', icon: <LungsIcon className="h-6 w-6 text-red-300 animate-bounce" />, title: 'Use Inhaler Now!', message: `Emergency alert in ${Math.floor(locationShareCountdown / 60)}:${(locationShareCountdown % 60).toString().padStart(2, '0')} if inhaler not detected.`, cancel: true },
        permissionDenied: { bg: 'bg-yellow-900', border: 'border-yellow-500', text: 'text-yellow-200', icon: <WarningIcon className="h-6 w-6 text-yellow-300" />, title: 'Location Denied', message: 'Cannot share location.', cancel: false },
        sent: { bg: 'bg-green-900', border: 'border-green-500', text: 'text-green-200', icon: <BellIcon className="h-6 w-6 text-green-300" />, title: 'Alert Sent', message: `SMS sent to Dr. Reed with Lat: ${userLocation?.latitude.toFixed(4)}, Long: ${userLocation?.longitude.toFixed(4)}`, cancel: false },
        ...
        cancelledInhalerUse: { bg: 'bg-sky-900', border: 'border-sky-500', text: 'text-sky-200', icon: <LungsIcon className="h-6 w-6" />, title: 'Inhaler Use Detected', message: 'Emergency countdown cancelled. Good job.', cancel: false }
    }[locationShareState];
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                     <div className="bg-dark-surface p-6 rounded-xl border border-dark-border shadow-lg">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <span className={`px-2 py-1 rounded text-xs font-mono font-bold tracking-wider ${isSimulating ? 'bg-green-500/20 text-green-400 animate-pulse' : 'bg-slate-700 text-slate-400'}`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex space-x-4">
                                <button onClick={isSimulating ? stopSimulation : startSimulation} className={`flex-1 py-3 rounded-lg font-bold transition-all shadow-lg hover:shadow-cyan-500/20 ${isSimulating ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-gradient-to-r from-brand-blue to-brand-cyan text-slate-900 hover:scale-[1.02]'}`}>
                                    {isSimulating ? 'Stop Simulation' : 'Start Simulation'}
                                </button>
                                <button onClick={toggleSimulationCondition} disabled={!isSimulating} className={`flex-1 py-3 rounded-lg font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${simulationConditionRef.current === 'attack' ? 'bg-red-600 animate-pulse text-white' : 'bg-slate-700 text-white hover:bg-red-600/80'}`}>
